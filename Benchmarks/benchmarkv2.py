...
def classify(fn, payload: Any) -> tuple[bool, str]:
    try:
        fn(payload)
        return True, ""
    except Exception as exc:  # noqa: BLE001 — oracle harness: any raise == REJECT
...
def main():
    ...
    except Exception as exc:  # noqa: BLE001 — surface any import-time failure verbatim
    ...
    for case in cases:
        ...
        if strict_available and case.official_accept is False:
            if case.strict_accept is False:
                strict_feedback.append(feedback_score(case.mutation, case.strict_error))
    ...
    p(
        f"{'ID':7s} {'Base':22s} {'Oracle':7s} {'Lenient':8s} {'Strict':8s} {'Match':4s} Mutation"
    )
    p("─" * 110)

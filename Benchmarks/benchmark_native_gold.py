...
def global_mutations(base: str, payload: dict) -> list[tuple[str, Any]]:
    ...
    # unknown fields
    queue.append(("extra unknown root field", set_key(payload, "extra_unknown", True)))
    queue.append(("extra unknown root dict", set_key(payload, "extra_meta", {"x": 1})))
    return queue
...
def generate_cases(
    ...
    for i in range(combos):
        ...
        paths = [path for path, _ in iter_paths(payload) if path != "$._type"]
        if not paths:
            continue

...
def install(verbose: bool = True) -> bool:
    ...
    This SDK is standalone and does not monkey patch anything.
    This function exists only for compatibility and observability.
    """
    if verbose:
        log.info("Compatibility layer active in highrise_fast")
...
_OUTGOING_ENCODER_TYPES = frozenset(
    {
        "ChatRequest",
        "EmoteRequest",
        "ReactionRequest",
...
def _encode_compat_object(obj: Any) -> bytes:
    if isinstance(obj, dict):
        payload = dict(obj)
    else:
        payload = _object_payload(obj, add_type=True)
...
def get_encoder_for(cls):
    """
    Return a JSON encoder for known outgoing payload types.

    Raises NotImplementedError for unknown types so misuse fails loudly.
    """
    ...
    if cls is dict or name == "dict" or name in _OUTGOING_ENCODER_TYPES:
        return _encode_compat_object

    raise NotImplementedError(f"No encoder registered for {name!r}")
...
class Highrise:
    ...
    async def moderate_room(
        ...
        # Match official behavior more closely:
        # only include action_length for mute actions.
        if action == "mute" and action_length is not None:
            fields["action_length"] = action_length
...
async def _receive_loop(
    ...
    while True:
        ...
        if isinstance(rid, str):
            ...
            # If it is an Error with an unknown rid, fall through and treat
            # it as an unsolicited error.

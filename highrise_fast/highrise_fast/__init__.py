...
def _ensure_aiohttp() -> None:
    ...
    global WebSocketError
    global WSMsgType
    global WSServerHandshakeError
    ...
    try:
        _web_timeout = float(os.environ.get("HR_WEBAPI_TIMEOUT", "30") or 30)
    except (TypeError, ValueError):
        _web_timeout = 30.0

    WEB_TIMEOUT = (
        ClientTimeout(total=_web_timeout) if ClientTimeout is not None else None
    )
...
try:
    ...
    _BASE_EXCEPTION_GROUP = Exception
...
# ---------------------------------------------------------------------------
# sdk_fast / bootstrap compatibility
# ---------------------------------------------------------------------------
...
# Some loaders use alternate names.
patch = install
enable = install
setup = install
